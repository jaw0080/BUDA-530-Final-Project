...
We will now try a multinomial logit model using the multinom() function. We will follow Faraday's procedure
...
```
Only 46% of the data is correctly classified based on fitting the entire data set. Had we split the data into train and test subsets, we could expect the performance to be slightly worse since the model would be predicting new data. According to Faraday this performance is not unexpected when using multinomial logit models for classification problems. 
