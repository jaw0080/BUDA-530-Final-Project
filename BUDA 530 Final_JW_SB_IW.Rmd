...
A multinomial regression model is best to determine the appropriate risk levels and relevant predictors as they relate to a customer’s probability of being in default. The model is ordinal in nature, meaning there is a natural order that occurs through the distribution of default risk levels. It is that comparison of risk levels that is most important both in ordinal regression as well as to our customer in making decisions in extending credit to potential customers.
...
Having the ability to say the odds of moving from one risk level to another increase by a certain factor as the informational criteria increases or decreases provides the levers our stakeholder desires as well as an ability to automate based on correlation of the attributes that interest them most. 
...
A two-phase model utilizing binary logistic regression and time series analysis is implemented to evaluate credit offers and determine if potential customers are a risk to default on loans. This model is the best approach due to the dependent variable we are using for prediction only having two possible values – default or no default – as well as meeting the need to determine if seasonality impacts overall default volume.
...
In the second part of the model, a scan function is used to import the data related to variables such as credit history, payment history, and current credit liability. Once imported, a time series is created that investigates the frequency of customer payments relative to their due date, overall credit score on a monthly basis, and their debt-to-income ratio also on a monthly basis over a period of time determined by the financial institution (e.g. three years). Next, plots of the dataset and log function of the dataset provide a visual representation of quantity and occurrence of when defaults tend to happen most, to include on specific months of interest by the stakeholder. 
